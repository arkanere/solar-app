...
- Key tables: `geo_locations` (the country-scalable location reference), `business_profiles` + `business_accounts` (installer listings and their logins, split by concern and discriminated by `country_code`), `leaddata` (quote requests, with lifecycle category: unclaimed vs claimed), and `embeddings.in_embedding_index` (the RAG bookkeeping table, below). `businesses` is a read projection of `business_profiles`, on its way out. The per-country tables this list used to name — `locations`, `businesses_1`, `us_businesses` — are gone or archived (migrations 056–062).
