<script>
	...
	$: ({ dailySummary, recentEmails, stats, countryStats, filters, error } = data);
	...
	function applyFilters() {
		...
		window.location.href = '/callsafe/business-onboarding/invitation-email-campaign' + (query ? '?' + query : '');
	}

	function clearFilters() {
		window.location.href = '/callsafe/business-onboarding/invitation-email-campaign';
	}
</script>

<svelte:head>
	<title>Invitation Email Campaign - CallSafe Admin</title>
</svelte:head>
	...
	{#if countryStats.length > 0}
		<div class="section">
			...
			<div class="stats-grid small">
				{#each countryStats as cs}
					<div class="stat-card">
						<div class="stat-value">{cs.total_sent}</div>
						<div class="stat-label">{cs.country || 'Unknown'} - Sent</div>
						<div class="stat-sub">{cs.unique_recipients} unique</div>
					</div>
				{/each}
			</div>
		</div>
		...
		<div class="filters">
			...
			<label>
				...
				<select bind:value={filters.country}>
					...
					{#each countryStats as cs}
						<option value={cs.country}>{cs.country}</option>
					{/each}
				</select>
			</label>
					...
					<tbody>
						{#each dailySummary as row}
							<tr>
								...
								<td><span class="badge badge-{(row.country || '').toLowerCase()}">{row.country || '-'}</span></td>
					...
					<tbody>
						{#each recentEmails as email}
							<tr>
								...
								<td><span class="badge badge-{(email.country || '').toLowerCase()}">{email.country || '-'}</span></td>
								<td>{email.website || '-'}</td>
