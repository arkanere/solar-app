<script>
	...
	async function triggerWelcomeMail() {
		try {
			const response = await fetch('/api/triggerWelcomeMail', {
				method: 'POST',
				headers: {
	...
	async function triggerPasswordReset() {
		try {
			const response = await fetch('/api/triggerPasswordReset', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json'
				},
...
<main class={darkMode ? 'dark' : 'light'}>
	...
	<form method="POST">
		...
		<div class="form-group">
			...
			<input id="district" name="district" bind:value={updatedBusiness.district} required />
		</div>
