...
export async function POST({ request }) {
	try {
		...
		const blogCheck = await pool.query('SELECT id FROM in_blogs WHERE id = $1', [data.id]);
		...
		const slugCheck = await pool.query(
			'SELECT id FROM in_blogs WHERE slug = $1 AND id != $2',
			[data.slug, data.id]
		);
		...
		const existingBlog = await pool.query(
			'SELECT status, published_at FROM in_blogs WHERE id = $1',
			[id]
		);
