...
export async function load({ url }) {
	...
	try {
		...
		if (search) {
			conditions.push(`(vendor_name ILIKE $${paramIndex} OR recipient_email ILIKE $${paramIndex} OR contact_person ILIKE $${paramIndex})`);
			params.push(`%${search}%`);
			paramIndex++;
		}
		if (emailStatus === 'sent') {
		...
		const countResult = await pool.query(
			`SELECT COUNT(*) as total FROM masterlist_indian_businesses ${whereClause}`,
			params
		);
		...
		const businesses = await pool.query(
			...
			FROM masterlist_indian_businesses
			${whereClause}
			ORDER BY earlier_date DESC NULLS LAST
			LIMIT $${paramIndex} OFFSET $${paramIndex + 1}`,
			[...params, perPage, offset]
		);

		const states = await pool.query(
			`SELECT DISTINCT state FROM masterlist_indian_businesses WHERE state IS NOT NULL ORDER BY state`
		);
