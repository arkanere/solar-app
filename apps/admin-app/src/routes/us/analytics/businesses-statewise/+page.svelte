<script>
	...
	$: ({ selectedState, businesses, totalCount, stateAnalytics, error } = $page.data);
	...
	$: uniqueDistricts = businesses 
		? [...new Set(businesses.map(b => b.district).filter(d => d && d.trim() !== ''))]
			.sort()
		: [];
		...
		{#if selectedState}
					...
					<div class="breakdown-section">
						...
						<div class="status-grid">
							<div class="status-card tier1">
								<h4>Tier-1</h4>
								<div class="status-number">{stateAnalytics.statusBreakdown.tier1_count || 0}</div>
								<div class="status-description">Basic listings</div>
							</div>
							<div class="status-card tier2">
								<h4>Tier-2</h4>
								<div class="status-number">{stateAnalytics.statusBreakdown.tier2_count || 0}</div>
								<div class="status-description">Complete profiles</div>
							</div>
							<div class="status-card tier3">
								<h4>Tier-3</h4>
								<div class="status-number">{stateAnalytics.statusBreakdown.tier3_count || 0}</div>
			...
			<div class="business-section">
				<div class="business-header-section">
					<h3>All Businesses in {selectedState} ({filteredBusinesses.length}{#if selectedDistrict} in {selectedDistrict}{/if})</h3>
					
					{#if uniqueDistricts.length > 0}
						<div class="district-filter">
							<label for="district-filter">Filter by District:</label>
							<select id="district-filter" value={selectedDistrict} on:change={handleDistrictChange}>
							...
							<tbody>
								{#each filteredBusinesses as business}
									<tr>
										...
										<td class="email-cell">{business.email || 'Not provided'}</td>
										...
										<td class="actions-cell">
											<div class="table-actions">
												...
												{#if business.magic_link_token && business.slug}
													<a href="/business/{business.slug}/signin-link/{business.magic_link_token}" target="_blank" class="table-action-link access-account">Access Account</a>
												{/if}
											</div>
										</td>
									</tr>
