...
export async function POST({ request }) {
	try {
		...
		const blogCheck = await pool.query(
			'SELECT id, title FROM us_blogs WHERE id = $1',
			[data.id]
		);
		...
		await pool.query('DELETE FROM us_blogs WHERE id = $1', [data.id]);
...
export async function PATCH({ request }) {
	try {
		...
		const result = await pool.query(
			`UPDATE us_blogs SET status = 'archived' WHERE id = $1 RETURNING id, title`,
			[data.id]
		);
