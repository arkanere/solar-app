<script>
	...
	async function deleteBlog(blogId, blogTitle) {
		...
		try {
			const response = await fetch('/us/api/blogs/delete', {
				method: 'POST',
...
<div class="blogs-page" class:dark={darkMode}>
	<div class="page-header">
		...
		<button class="btn-create" on:click={() => goto('/us/blogs/create')}>
			+ Create New Blog
		</button>
	</div>
	...
	{#if filteredBlogs.length === 0}
		<div class="empty-state">
			...
			{#if blogs.length === 0}
				<button class="btn-primary" on:click={() => goto('/us/blogs/create')}>
						...
						<tr>
							<td class="title-cell">
								<div class="title-wrapper">
									<a href="/us/blogs/{blog.id}/edit" class="blog-title">{blog.title}</a>
									{#if blog.excerpt}
							...
							<td class="actions-cell">
								<div class="action-buttons">
									<button
										...
										on:click={() => goto(`/us/blogs/${blog.id}/edit`)}
										title="Edit"
									>
										✏️
									</button>
									{#if blog.status === 'published'}
										<a
											href="https://solarvipani.com/us/blogs/{blog.slug}"
											target="_blank"
