...
<section id="lead-data">
	<h2 class="text-2xl mb-4 font-semibold text-left text-accent">Customer Inquiry</h2>
	{#if errorMessage}
	...
	{:else}
				...
				<li class="mb-6 p-0 rounded-xl shadow-sm break-words transition-shadow duration-200 overflow-hidden list-none opacity-70 border-2 border-dashed border-border bg-card">
					...
					<div class="p-5 space-y-3">
						...
						<div class="flex flex-wrap items-center gap-4 max-sm:flex-col max-sm:items-start">
							...
							<button
								class="inline-flex items-center gap-2 py-2.5 px-5 text-sm font-semibold border-none rounded-md cursor-not-allowed transition-all bg-muted text-muted-foreground opacity-60"
								disabled
		...
		<!-- Open CRM Button -->
		<div class="flex justify-center mt-6 pt-5 border-t-2 border-border">
			<a
				href="/in/{businessSlug}/crm"
				class="inline-block text-center min-w-[120px] py-3 px-8 rounded-lg font-semibold text-base no-underline transition-all bg-accent text-accent-foreground shadow-md hover:bg-accent-hover hover:-translate-y-0.5 hover:shadow-lg"
			>
				Open CRM
			</a>
		</div>
