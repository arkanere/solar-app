<script lang="ts">
	...
	import { Phone, Clock } from '@lucide/svelte';
	...
	function getCategoryBadge(category: number | null) {
		if (category === 1) {
			...
			return { label: 'Non-Exclusive-Claimed', variant: 'default' };
		} else {
			return { label: 'Exclusive', variant: 'outline' };
		}
	}

	function getNextAction(stage: number, category: number | null, status: boolean) {
		...
		filteredLeads = leads.filter((lead) => {
			if (selectedCategory !== 'all') {
				...
				if (categoryValue === 3) {
					...
					if (lead.category !== categoryValue) return false;
				}
			}
	...
	function confirmDelete() {
		if (leadToDelete) deleteLead(leadToDelete);
	}
</script>

<section id="lead-data" class="space-y-6">
	<h2 class="text-2xl font-semibold text-accent">Customer Inquiry</h2>

	{#if errorMessage}
		<p class="text-destructive font-bold">{errorMessage}</p>
	{:else}
		{#if leads.length > 0}
			<LeadStageFilter
			...
			{#if filteredLeads.length === 0}
				<Card.Root class="border-dashed border-2">
					<Card.Content class="pt-6 text-center">
						<p class="font-semibold text-lg text-muted-foreground">
					...
					<Card.Root>
						<Card.Header>
							<div class="flex items-center justify-between gap-2 flex-wrap">
								...
								<Badge variant={getCategoryBadge(lead.category).variant}>
									{getCategoryBadge(lead.category).label}
								</Badge>
							</div>
						</Card.Header>
						<Card.Content class="space-y-4">
							...
							<!-- Stage and Status Controls (for claimed/exclusive leads) -->
							{#if lead.category !== 1}
