<script lang="ts">
	...
	import AddReferrer from '$lib/components/AddReferrer.svelte';
	...
	// main-app still carries a [country] segment, so the referrer link below takes
	...
	//
	// ⚠️ This URL shape appears to be dead: main-app has no `/referrer/` route and
	// no rewrite that produces one, for either country. Left as-is rather than
	// guessed at — only the country segment is made dynamic here, which is step
	// C's job. Fixing the shape is its own task.
	function getReferrerLink(referrerSlug: string) {
		return `https://solarvipani.com/${country}/solar-panel-installer/${businessSlug}/referrer/${referrerSlug}`;
	}
	...
	async function deleteReferrer(referrerId: number, referrerName: string) {
		...
		try {
			const response = await fetch('/api/deleteReferrer', {
				method: 'POST',
