...
import { IN_LEAD_COLUMNS } from '$lib/server/unifiedRead';
...
export const load: PageServerLoad<PageData> = async ({ params }) => {
	...
	try {
		...
		const businessResult = await pool.query(
			`SELECT source_id AS id, businessname, description, phonenumber, email, address, website, level2 AS district, level1 AS state
