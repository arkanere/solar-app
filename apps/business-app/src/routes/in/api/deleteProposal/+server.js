...
export async function POST({ request, cookies }) {
	...
	try {
		...
		const checkQuery = `
			...
			WHERE id = $1
		`;
		const checkResult = await pool.query(checkQuery, [proposalId]);
		...
		`;
		const deleteResult = await pool.query(deleteQuery, [proposalId]);
		...
		if (error.message && error.message.includes('relation "proposals" does not exist')) {
			return json(
		...
		return json({ success: false, error: 'Failed to delete proposal' }, { status: 500 });
	} finally {
		await pool.end();
	}
}
