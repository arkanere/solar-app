...
export async function POST({ request, cookies }) {
	...
	try {
		...
		if (!referrerId || !businessId) {
			return json({ success: false, error: 'Referrer ID and Business ID are required' }, { status: 400 });
		}
		...
		`;
		const checkResult = await pool.query(checkQuery, [referrerId, businessId]);
		...
		const deleteQuery = `
			...
			RETURNING id, name
		`;
		const deleteResult = await pool.query(deleteQuery, [referrerId, businessId]);
