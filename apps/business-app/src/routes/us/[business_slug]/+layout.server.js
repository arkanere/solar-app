...
export const load = async ({ cookies, params, url }) => {
	...
	const isClaimPage = url.pathname === `/us/${business_slug}/claim`;
	const isLoginPage = url.pathname === `/us/${business_slug}/login`;
	const isResetPasswordPage = url.pathname.startsWith(`/us/${business_slug}/reset-password`);
	const isMagicLinkPage = url.pathname.startsWith(`/us/${business_slug}/signin-link`);
	...
	if (business_session && !isMagicLinkPage) {
		try {
			...
			if (sessionData.businessSlug !== business_slug) {
				throw redirect(302, `/us/${sessionData.businessSlug}`); // Redirect back to the correct business
			}
			...
			throw redirect(302, '/us/login'); // Redirect to login page
		}
	}
