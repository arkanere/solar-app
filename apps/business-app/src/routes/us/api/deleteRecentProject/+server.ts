...
import { usProjects } from '@solar/db/schema';
...
export const DELETE: RequestHandler = async ({ request, cookies }) => {
	try {
		...
		const [project] = await db
			.select({
				businessSlug: usProjects.businessSlug,
				cloudinaryPublicId: usProjects.cloudinaryPublicId
			})
			.from(usProjects)
			.where(eq(usProjects.id, Number(projectId)))
			.limit(1);
