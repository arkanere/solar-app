...
>
	...
	>
		...
		<Input type="email" id="email" name="email" required class="mb-6" />

		<Label for="password" class="mb-2 font-medium">Password:</Label>
		<Input type="password" id="password" name="password" required class="mb-6" />

		{#if form?.errors?.message}
			<p class="text-destructive text-sm mb-4">{form.errors.message}</p>
		{/if}

		<Button type="submit">Login</Button>

		<a href="/us/forgot-password" class="text-sm text-center mt-4 underline">Forgot your password?</a>
	</form>
</main>
