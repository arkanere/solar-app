...
<Dialog.Root bind:open={$storiesModalOpen}>
  <Dialog.Content class="fixed inset-0 z-50 flex items-center justify-center bg-black/95 border-0 p-0 rounded-0 w-full max-w-none h-full max-h-none">
    <!-- Loading State -->
    {#if $storiesLoading}
      <div class="flex flex-col items-center justify-center w-full max-w-sm gap-4 px-3">
    ...
    {:else if $storiesError}
      <Card class="w-full max-w-sm bg-[hsl(var(--destructive)/0.1)] border-[hsl(var(--destructive)/0.2)]">
        <CardHeader>
    ...
    {:else}
      ...
      <div class="relative w-full h-full overflow-hidden flex flex-col bg-gradient-to-b from-black/80 to-black/40 max-w-sm" style="max-height: 700px;">
        <!-- Progress Bars -->
        <div class="flex absolute top-0 left-0 right-0 z-40 gap-1 p-3">
        ...
        <!-- Story Header -->
        <div class="flex items-center justify-between relative z-30 p-3 pt-16">
        ...
        <div class="flex-1 flex flex-col items-center justify-center bg-black/30 min-h-0 relative">
          {#if getImageUrl($storiesData[currentStoryIndex], PUBLIC_CLOUDINARY_CLOUD_NAME, "w_800,h_800")}
            <img
              ...
              class="w-full h-auto object-contain mt-3 max-h-[70vh]"
            />
          {:else}
        ...
        <!-- Navigation Areas -->
        <div class="absolute left-0 top-1/2 -translate-y-1/2 z-20 ml-3">
          <Tooltip>
            <TooltipTrigger>
              <Button
        ...
        <div class="absolute right-0 top-1/2 -translate-y-1/2 z-20 mr-3">
          <Tooltip>
            <TooltipTrigger>
              <Button
