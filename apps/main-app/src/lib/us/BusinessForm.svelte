<script>
	...
	async function updateCounties(selectedState) {
		...
		try {
			...
			const res = await fetch('/api/us/getCounties', {
				method: 'POST',
	...
	async function updateCities(selectedCounty) {
		...
		try {
			...
			const res = await fetch('/api/us/getCities', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
	...
	function validateEIN() {
		const einRegex = /^\d{2}-?\d{7}$/; // EIN format: XX-XXXXXXX or XXXXXXXXX
		if (!einRegex.test(gstn)) {
			errors.gstn =
				'EIN must be in format XX-XXXXXXX or 9 digits';
			return false;
		} else {
			errors.gstn = '';
			return true;
	...
	async function handleSubmit(event) {
		...
		if (validatePhoneNumber() && validateWhatsappNumber() && validateEIN()) {
			isSubmitting = true;
			try {
				const response = await fetch('/api/us/submitBusiness', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
						businessName,
...
<form on:submit|preventDefault={handleSubmit}>
	...
	<!-- EIN Input -->
	<div>
		<label for="gstn">EIN (Employer Identification Number):</label>
		<input
			id="gstn"
			type="text"
			bind:value={gstn}
