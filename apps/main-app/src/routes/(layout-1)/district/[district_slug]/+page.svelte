...
<svelte:head>
	<title>Solar Panel Installers in {district} - Find installers by city - Solar Vipani</title>
	<meta
		name="description"
	...
	{:else}
		...
		<ul class="cities-grid">
			{#each cities as city}
				<li>
					<a
						href={`/solar-panel-installer-directory/${formatCitySlug(city)}`}
						rel="noopener"
						class="city-card"
					>
						<h2>{city}</h2>
						<p>Find solar installers in {city}</p>
						<div class="view-more">
