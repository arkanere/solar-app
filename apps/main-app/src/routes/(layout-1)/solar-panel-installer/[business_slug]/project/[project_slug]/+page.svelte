<script>
	...
	const navigateToBusinessPage = () => {
		goto(`/solar-panel-installer/${business.slug}`);
	};

	const navigateToDirectory = () => {
		goto(`/solar-panel-installer-directory/${encodeURIComponent(business.city)}`);
	};
</script>

<svelte:head>
	<title
		>{project?.title || 'Project'} by {business?.businessname || 'Business'} | Solar Vipani</title
	>
	<meta
		name="description"
		content={project
	...
	<meta property="og:title" content="{project?.title} by {business?.businessname}" />
	<meta
...
<main class={darkMode ? 'dark' : 'light'}>
	...
	<section class="project-image-section">
		...
		{:else if project.image_url}
			<img src={project.image_url} alt={project.title} class="project-main-image" loading="eager" />
		{:else}
