<script lang="ts">
	...
	import StateSubsidyPage from '$lib/components/seo/StateSubsidyPage.svelte';
	import DiscomPage from '$lib/components/seo/DiscomPage.svelte';
	...
	const breadcrumbItems = $derived.by(() => {
		...
		if (isDiscom && d.stateSubsidy) {
			items.push({
				...
				url: `https://solarvipani.com/solar-subsidy/${d.discom.state_slug}`
			});
		}
		items.push({
			name: isCluster ? d.clusterData.h1 : isState ? d.subsidy.state_name : d.discom.name,
			url: `https://solarvipani.com/solar-subsidy/${canonicalSlug}`
		});
		return items;
	});
...
<svelte:head>
	...
	<link rel="canonical" href="https://solarvipani.com/solar-subsidy/{canonicalSlug}" />
