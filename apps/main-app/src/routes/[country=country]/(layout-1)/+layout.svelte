...
<!-- Chatbot: launcher, lazy popup and scroll auto-open all live in ChatDock,
     which the country-less root layout renders too. `features.chatbot` is true
     for every country today, but it stays as the per-country capability gate. -->
{#if features.chatbot}
  <ChatDock />
{/if}
