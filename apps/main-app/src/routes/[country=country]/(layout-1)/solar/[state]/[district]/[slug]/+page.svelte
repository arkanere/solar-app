<script lang="ts">
	...
	const pageTitle = $derived(
		isCity ? `Solar Panel Installers in ${d.city}, ${d.level2}`
		: isBrand ? `${d.brandName} Solar Installers in ${d.level2}`
		: `${d.sizeKw}kW Solar System in ${d.level2}, ${d.level1}`
	);

	const metaDescription = $derived(
		isCity ? `Find ${d.installerCount} solar installers in ${d.city}, ${d.level2}. Compare quotes and view recent projects.`
		: isBrand ? `Find ${d.brandName} solar panel installers in ${d.level2}, ${d.level1}. Compare verified installers.`
	...
	const breadcrumb = $derived(breadcrumbLD([
		...
		{ name: isCity ? d.city : isBrand ? d.brandName : `${d.sizeKw}kW Solar`,
		  url: `https://solarvipani.com/${cc}/solar/${d.level1Slug}/${d.level2Slug}/${isCity ? d.citySlug : isBrand ? d.brandSlug : `${d.sizeKw}kw-solar-system`}` }
	]));
	...
	const businessLDs = $derived(
		(d.businesses || []).slice(0, 5).map((b: Record<string, any>) =>
			localBusinessLD({
				...
				city: (b.city as string) || d.level2,
				state: d.level1,
	...
	const canonicalUrl = $derived(`https://solarvipani.com/${cc}/solar/${d.level1Slug}/${d.level2Slug}/${canonicalSlug}`);
	const locationName = $derived(isCity ? d.city : d.level2);
...
<svelte:head>
	...
	<meta name="geo.placename" content="{locationName}, {d.level1}" />
...
<div class="max-w-6xl mx-auto px-4 py-8">
	...
	{#if isSize}
		<section class="mb-8 bg-accent/10 rounded-lg p-6">
			...
			{:else}
				<p class="text-muted-foreground">Contact an installer for {d.sizeKw}kW system pricing in {d.level2}.</p>
			{/if}
		</section>
	{/if}
	...
	<!-- Subsidy (city only) -->
	{#if isCity && country.features.subsidy}
		...
		<section class="mb-8">
			...
			<div class="flex flex-wrap gap-2">
				{#each d.siblingCities as sc}
					<a
						href="/{cc}/solar/{d.level1Slug}/{d.level2Slug}/{sc.slug}/"
		...
		<section class="mb-8">
			...
			<div class="flex flex-wrap gap-2">
				{#each COMMON_SIZES.filter(s => s !== d.sizeKw) as size}
					<a
						href="/{cc}/solar/{d.level1Slug}/{d.level2Slug}/{size}kw-solar-system/"
						class="bg-muted hover:bg-accent/20 text-sm rounded-lg px-3 py-1.5 transition-colors"
					>
	...
	{#if cc === 'in'}
		<div class="text-center mb-8">
			...
			>
				Get Free Solar Quotes in {isCity ? d.city : d.level2}
			</a>
		</div>
	{/if}
