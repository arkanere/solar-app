...
<main class="min-h-screen pt-2 flex flex-col items-center justify-start text-center transition-colors duration-[theme(--transition-default)] bg-background text-foreground">
	...
	{:else}
		...
		{#if customerDetails}
		<div class="w-full max-w-2xl mx-auto mb-8 px-2 text-left">
			<Card class="p-2 md:p-4 border border-border">
				<div class="space-y-3">
					<div class="flex flex-col md:flex-row md:justify-between md:items-start pb-3 border-b border-border last:border-b-0">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Name:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{customerDetails.name}</span>
					</div>
					<div class="flex flex-col md:flex-row md:justify-between md:items-start pb-3 border-b border-border last:border-b-0">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Phone:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{customerDetails.phone}</span>
					</div>
					<div class="flex flex-col md:flex-row md:justify-between md:items-start pb-3 border-b border-border last:border-b-0">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Email:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{customerDetails.email || 'Not provided'}</span>
					</div>
					<div class="flex flex-col md:flex-row md:justify-between md:items-start pb-3 border-b border-border last:border-b-0">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Location:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{customerDetails.pinCode}{customerDetails.district ? `, ${customerDetails.district}` : ''}</span>
					</div>
					<div class="flex flex-col md:flex-row md:justify-between md:items-start pb-3 border-b border-border last:border-b-0">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Installation Type:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{customerDetails.type}</span>
					</div>
					<div class="flex flex-col md:flex-row md:justify-between md:items-start pb-3 border-b border-border last:border-b-0">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Requirements:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{customerDetails.comment}</span>
					</div>
					<div class="flex flex-col md:flex-row md:justify-between md:items-start">
						<span class="font-semibold text-foreground-secondary md:min-w-[140px] flex-shrink-0">Submitted:</span>
						<span class="font-medium text-foreground md:text-right flex-grow md:ml-4">{formatDate(customerDetails.submittedAt)}</span>
					</div>
				</div>
			</Card>
		</div>
		...
		{#if customerDetails && !customerDetails.hasVerifiedBusinessInDistrict && customerDetails.district}
			<Alert class="w-full max-w-2xl mx-auto mb-8 px-2 py-2 bg-warning/10 border border-warning rounded-[theme(--radius-lg)] shadow-[theme(--shadow-sm)] text-center">
				<h3 class="text-lg md:text-xl font-semibold mb-2 text-warning">Service Area Update</h3>
				<p class="text-sm md:text-base text-warning/90 leading-relaxed">We are expanding to <strong>{customerDetails.district}</strong>. We will reach out to you once we have a verified installer there.</p>
			</Alert>
		{/if}
	{/if}
