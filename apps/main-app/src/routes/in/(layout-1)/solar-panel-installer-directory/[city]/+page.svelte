<script>
	...
	import { city_jsonLD1 } from '$lib/city_jsonLD1';
	import LeadFormModal from '$lib/LeadFormModal.svelte';
	import LeadFormBusiness from '$lib/LeadFormBusiness.svelte';
	import BusinessTilesList from '$lib/BusinessTilesList.svelte'; // Keep for critical content
	import RecommendedSolarSystems from '$lib/RecommendedSolarSystems.svelte';
	import SolarComparisonTable from '$lib/SolarComparisonTable.svelte';
	...
	function createJsonLD() {
		...
		const city_jsonLD_2 = (businesses.length > 0 ? businesses : [defaultBusiness]).map(
			(business) => ({
				...
				url: `https://solarvipani.com/solar-panel-installer/${business.slug}`,
				aggregateRating: {
		...
		const breadcrumbSchema = {
			...
			itemListElement: [
				...
				},
				{
					...
					item: 'https://solarvipani.com/solar-panel-installer-directory'
				},
				{
					'@type': 'ListItem',
					position: 3,
					name: `Solar Panel Installers in ${city.replace('-', ' ')}`,
					item: `https://solarvipani.com/solar-panel-installer-directory/${city.toLowerCase()}`
				}
			]
		};
...
<svelte:head>
	<title
		>Top Solar Panel Installers in {city.replace('-', ' ')} | Know Price | Verified Experts and Dealers</title
	>
	<meta
		name="description"
	...
	<link
		rel="canonical"
		href="https://solarvipani.com/solar-panel-installer-directory/{city.toLowerCase()}"
	/>
	...
	<meta
		property="og:title"
		content="Top Solar Panel Installers in {city.replace('-', ' ')} | Solar Vipani"
	/>
	<meta
	...
	<meta
		property="og:url"
		content="https://solarvipani.com/solar-panel-installer-directory/{city.toLowerCase()}"
	/>
	<meta property="og:image" content="https://solarvipani.com/logo.webp" />
	<meta property="og:image:alt" content="Solar panel installers in {city.replace('-', ' ')}" />
	...
	<meta name="twitter:image:alt" content="Solar panel installers in {city.replace('-', ' ')}" />
...
<main class={darkMode ? 'dark' : 'light'}>
	<h1>Best Solar Panel Installers in {city.replace('-', ' ')} - Top Rated Companies Near You</h1>
	{#if lastUpdated}
		...
		<section id="no-installers" class="combined-recruitment-section">
			<div class="combined-content">
				...
				<button
					...
					on:click={() => (window.location.href = '/business-form')}
				>
	...
	<section id="lead-form-sv">
		{#if businesses.length === 0}
			<h2>Be the First to Get Quotes When Installers Join {city.replace('-', ' ')}</h2>
			<p class="lead-form-subtitle">
				Submit your requirements and we'll notify you as soon as verified installers become
				available in your area.
			</p>
		{:else}
			<h2>Quickly Get Quotation from 2-3 Verified Solar Installers in {city.replace('-', ' ')}</h2>
			<p class="privacy-note">Be assured, we follow highest privacy standards to keep your details safe. <a href="/privacy-policy">Learn more</a></p>
		{/if}
		<LeadFormBusiness />
	</section>
		...
		<div class="services-content">
			<div class="service-item">
				...
				<p>
					...
					industrial needs. Commonly used systems for independant homes and bunglows are <strong
						><a href="/blogs/1kw-ongrid-solar-system">1kW ongrid solar system</a>,
						<a href="/blogs/3kw-ongrid-solar-system">3kW ongrid solar system</a>,</strong
					>
					and <strong><a href="/blogs/5kw-ongrid-solar-system">5kW ongrid solar system</a>.</strong>
				</p>
			</div>
			...
			<div class="service-item">
				...
				<p>
					...
					financial companies. Nowadays <strong
						><a href="/blogs/zero-cost-emi-schemes-solar">Zero cost EMI schemes</a></strong
					> have become popular.
				</p>
			</div>
			...
			<ul>
				<li>
					...
					<p>
						...
						For instance, a 3kW system typically ranges from ₹1.5 lakhs to ₹2.5 lakhs, including installation
				...
				<li>
					...
					<p>
						A 3kW solar panel system in {city} generally costs between ₹1.5 lakhs to ₹2.5 lakhs, depending
						on the brand and installation factors.
					</p>
				</li>
				<li>
					<strong>How much money will I save if I install solar panels?</strong>
					<p>
						If you install solar panels with net metering, you'll save approximately ₹5.5 to ₹6.5
						per unit compared to the ₹8-₹9 per unit cost from the DISCOM. Over 25 years, this
						translates to substantial savings, making solar energy a cost-effective and sustainable
						choice.
					</p>
				</li>
			</ul>
	...
	{#if district}
		<section class="district-link-section">
			...
			<p>
				If you're interested in exploring other solar businesses in {district}, visit our district
				directory page.
			</p>
			<button
				on:click={() =>
					(window.location.href = `/district/${district.toLowerCase().replace(/\s+/g, '-')}`)}
			>
