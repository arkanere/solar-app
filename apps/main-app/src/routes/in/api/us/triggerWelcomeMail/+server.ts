...
export const POST: RequestHandler = async ({ request, fetch }) => {
	...
	try {
		...
		const magicTokenResponse = await fetch('/api/us/createMagicLinkToken', {
			method: 'POST',
		...
		const subject = `Welcome to Solar Vipani USA, ${businessname}! LOGIN to your account Now`;
		const message = `
      <!DOCTYPE html>
        <html lang="en">
        <head>
            ...
            <title>Welcome to Solar Vipani USA</title>
        </head>
        <body>
