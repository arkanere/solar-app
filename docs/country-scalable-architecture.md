...
4. **Retire old tables** — only after all apps (incl. the external admin-app) migrate: drop triggers, keep old tables as archive or drop.
   ...
   **Runbook (2026-07-19; 049 + drift check WRITTEN, NOT applied):**
   ...
   5. **040 triggers stay for now** (`businesses_1 → in_business_profiles`/`in_business_accounts`): business-app IN writes still target `businesses_1` (`countryTables.ts: in: 'businesses_1'`) and rely on them; admin-app reads the split tables and `sv_sync_business('in')` sources from `in_business_profiles`. Next code slice: repoint business-app IN writes at the split tables (admin-app pattern: split-table write + `businesses_1` dual-write), then drop the 039/040 triggers.
