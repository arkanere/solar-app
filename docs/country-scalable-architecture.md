...
4. **Retire old tables** — only after all apps (incl. the external admin-app) migrate: drop triggers, keep old tables as archive or drop.
   ...
   **Progress on step 6 (2026-07-19):** `051-drop-split-sync-triggers.sql` **APPLIED** — the live DB is now fully trigger-free; every sync is an explicit app call (verified: bare `businesses_1` UPDATE no longer propagates, app path does, drift 0). **admin-app read-switch DONE** (solar-app-internal): zero reads of `leaddata`/`us_leaddata`/`us_businesses` remain anywhere in admin-app — all 27 converted pages verified 200 against the live DB via dev server with a minted session. **Remaining legacy readers before freeze:** business-app `claimLead`'s branch-credential copy reads `businesses_1` directly, and every app still *writes* legacy-first. **Final cutover design (agreed direction, not yet implemented):** repoint writes to unified/split tables directly, minting `source_id` from the shared `leaddata_id_seq` (and businesses_1/us_businesses seqs) so `(country_code, source_id)` identity, claim-request FKs, and `source_id AS id` reads keep working; repoint `sv_sync_account('in')` to read `in_business_accounts` once `businesses_1` freezes; then rename legacy tables to `*_archive`.
...
3. **IN businesses/accounts**: `in_business_profiles`/`in_business_accounts` become the sole write targets (they already are for main-app/admin-app profile+token paths; business-app auth/`countryTables.ts: in` and the `businesses_1` id-minting in main-app submitBusiness must repoint, minting ids from the `businesses_1` sequence). Repoint `sv_sync_account('in')`/`sv_sync_business('in')` to source from the split tables (they currently read `businesses_1`/`in_business_profiles` — only the account one changes). Fix business-app `claimLead`'s branch-credential copy to read the split tables (last legacy read anywhere).
4. **Migration 052**: after a drift check and the flips above are deployed, rename `leaddata`/`us_leaddata`/`businesses_1`/`us_businesses` to `*_archive` (keeps history queryable, guarantees no straggler writer goes unnoticed — anything still touching the old names fails loudly). Sequences stay (they mint `source_id`).
5. **Cleanup pass**: delete the now-unused `unifiedSync` helpers and `sv_sync_*` SQL functions, remove the `TODO(remove after ...)` dual-write comments, and (optional, cosmetic) migrate readers off the legacy column aliases at leisure.

Independent housekeeping, any time: the modified `automation-scripts/` files in solar-app-internal are uncommitted pre-existing work (outside admin-app scope) — review and commit or discard; `plan.md` at the solar-app repo root looks like a scratch file — delete if unneeded.
